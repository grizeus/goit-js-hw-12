...
describe('renderGallery', () => {
  ...
  it('should render a single picture correctly', () => {
    ...
    expect(result).toContain(`<img class="gallery-image" src="${he.encode(mockPicture.webformatURL)}" alt="${he.encode(mockPicture.tags.join(', '))}" />`);
    expect(result).toContain(`<li class="gallery-item-likes"><b>Likes</b> ${mockPicture.likes}</li>`);
    expect(result).toContain(`<li class="gallery-item-views"><b>Views</b> ${mockPicture.views}</li>`);
    expect(result).toContain(`<li class="gallery-item-comments"><b>Comments</b> ${mockPicture.comments}</li>`);
    expect(result).toContain(`<li class="gallery-item-downloads"><b>Downloads</b> ${mockPicture.downloads}</li>`);
  });

  it('should render multiple pictures correctly', () => {
    const mockData = {
      hits: [mockPicture, { ...mockPicture, webformatURL: 'https://example.com/small2.jpg' }]
    };
  ...
  it('should handle pictures with missing properties', () => {
    const incompletePicture = { ...mockPicture, likes: undefined, comments: undefined };
    const result = renderGallery({ hits: [incompletePicture] });
    expect(result).toContain('<li class="gallery-item-likes"><b>Likes</b> undefined</li>');
    expect(result).toContain('<li class="gallery-item-comments"><b>Comments</b> undefined</li>');
  });

  it('should escape HTML in tags', () => {
    const pictureWithHtmlTags = { ...mockPicture, tags: ['<script>alert("xss")</script>'] };
  ...
  it('should handle tags as an array and escape HTML in tags and URLs', () => {
    ...
    const result = renderGallery({ hits: [pictureWithHtmlTags] });
    expect(result).not.toContain('<script>');
    expect(result).toContain(he.escape('<script>alert("xss")</script>, nature'));
    expect(result).toContain(he.escape('https://example.com/small.jpg?param=<script>'));
    expect(result).toContain(he.escape('https://example.com/large.jpg?param=<script>'));
  });
